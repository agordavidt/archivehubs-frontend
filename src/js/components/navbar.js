...
function renderPopup() {
  ...
  if (active) {
    ...
    linkEl.href = '/pages/profile.html';
  } else {
  ...
  if (navAvatar && active) {
    navAvatar.src = getAccountAvatar(active);
  }
...
function highlightActiveNav() {
  ...
  document
    .querySelectorAll('.nav-link[data-nav], .mobile-nav-link[data-nav]')
    .forEach((link) => {
...
function profileUrlFor(account) {
  if (!account) return '/pages/profile.html';

  return account.account_type === 'corporate'
    ? `/pages/profile.html?id=${account.id}&type=corporate`
    : `/pages/profile.html?id=${account.id}`;
}
